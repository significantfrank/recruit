...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"12px 6px","margin":"172px 0 0","borderColor":"#ddd","alignItems":"center","display":"flex","clear":"both","float":"right","justifyContent":"center","borderRadius":"12px","flexWrap":"wrap","background":"#fff","borderWidth":"0 0 0px","width":"380px","borderStyle":"solid","height":"auto","order":"1"}'>
      <el-form-item :style='{"width":"100%","margin":"0 0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"120px","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>职位名称：</div>
        <el-input v-model="formSearch.zhiweimingcheng" placeholder="职位名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"width":"100%","margin":"0 0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"120px","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>工作地点：</div>
        <el-input v-model="formSearch.gongzuodidian" placeholder="工作地点" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"width":"100%","margin":"0 0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"120px","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>薪资待遇：</div>
      ...
      <el-form-item :style='{"width":"100%","margin":"0 0 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"120px","padding":"0 0 0 0px","lineHeight":"42px","textAlign":"right","display":"inline-block"}'>公司名称：</div>
        <el-input v-model="formSearch.gongsimingcheng" placeholder="公司名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
	  <el-button v-if=" true " :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0px 20px 0 0px","color":"#fff","display":"inline-block","outline":"none","borderRadius":"30px","background":"#1e3c4f","width":"100px","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="getList(1, curFenlei)"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-search"></i>查询</el-button>
	  <el-button v-if="btnAuth('zhiweixinxi','新增')" :style='{"cursor":"pointer","border":"0px solid #ddd","padding":"0px 15px","margin":"0px 0px 0 0","color":"#fff","display":"inline-block","outline":"none","borderRadius":"30px","background":"#1c6a6d","width":"100px","fontSize":"14px","lineHeight":"36px","height":"36px"}' type="primary" @click="add('/index/zhiweixinxiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"minHeight":"900px","padding":"20px 0","margin":"0px 0 0","borderRadius":"0px","background":"none","width":"calc(100% - 400px)","float":"left","order":"6"}'>
		...
		<div :style='{"width":"100%","padding":"0 0 0 2%","flexWrap":"wrap","background":"none","display":"flex","height":"auto"}' class="list list4 index-pv1">
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" :style='{"width":"31.3%","margin":"0 2% 30px 0","overflow":"hidden","position":"relative","background":"#fff","height":"480px"}' class="item">
			<img @click.stop="imgPreView(item.gongsitupian)" v-if="item.gongsitupian && item.gongsitupian.substr(0,4)=='http'" :src="item.gongsitupian.split(',')[0]" class="image" />
			<img @click.stop="imgPreView(baseUrl + (item.gongsitupian?item.gongsitupian.split(',')[0]:''))" v-else :src="baseUrl + (item.gongsitupian?item.gongsitupian.split(',')[0]:'')" class="image" />
			<div class="content">
		      <div :style='{"padding":"30% 10px 20px 0","borderColor":"#ddd","color":"inherit","alignItems":"center","display":"block","textIndent":"2em","overflow":"hidden","borderWidth":"0 1px 0 0","width":"48%","lineHeight":"30px","fontSize":"inherit","borderStyle":"dashed","height":"100%"}'>{{item.zhiweiyaoqiu}}</div>
		      <div class="info" :style='{"padding":"30% 0 0 0px","overflow":"hidden","alignItems":"flex-start","flexWrap":"wrap","display":"block","width":"48%","height":"100%"}'>
				<div :style='{"padding":"0 0 0 20px","borderColor":"#ddd","whiteSpace":"nowrap","color":"inherit","textAlign":"left","overflow":"hidden","borderWidth":"0 0 0px","width":"100%","lineHeight":"40px","fontSize":"inherit","textOverflow":"ellipsis","borderStyle":"dashed","fontWeight":"600","height":"auto"}' class="name ">{{item.zhiweimingcheng}}</div>
				<div :style='{"padding":"0 0 0 20px","borderColor":"#ddd","whiteSpace":"nowrap","color":"inherit","textAlign":"left","overflow":"hidden","borderWidth":"0 0 0px","width":"100%","lineHeight":"40px","fontSize":"inherit","textOverflow":"ellipsis","borderStyle":"dashed","fontWeight":"600","height":"auto"}' class="name ">{{item.zhiweileixing}}</div>
				<div :style='{"padding":"0 0 0 20px","borderColor":"#ddd","whiteSpace":"nowrap","color":"inherit","textAlign":"left","overflow":"hidden","borderWidth":"0 0 0px","width":"100%","lineHeight":"40px","fontSize":"inherit","textOverflow":"ellipsis","borderStyle":"dashed","fontWeight":"600","height":"auto"}' class="name ">{{item.gongzuodidian}}</div>
		        ...
		        <div :style='{"width":"100%","padding":"0 0 0 20px","borderColor":"#ddd","borderStyle":"dashed","borderWidth":"0 0 1px","display":"none"}'>
		          ...
		          <span :style='{"color":"inherit","lineHeight":"40px","fontSize":"inherit"}'>{{item.gongsizhanghao}}</span>
		        </div>
		        <div :style='{"width":"100%","padding":"0 0 0 20px","borderColor":"#ddd","borderStyle":"dashed","borderWidth":"0 0 0px"}'>
...
<script>
  export default {
    ...
    async created() {
		...
		await this.$http.get('option/zhiweileixing/zhiweileixing').then(res => {
			if (res.data.code == 0) {
				this.zhiweileixingOptions = res.data.data;
				this.selectOptionsList.push({name:'职位类型',list:this.zhiweileixingOptions,tableName: 'zhiweileixing',check: -1})
			}
		});
      this.getFenlei();
      this.getList(1, '全部');
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.zhiweimingcheng != '') searchWhere.zhiweimingcheng = '%' + this.formSearch.zhiweimingcheng + '%';
        if (this.formSearch.zhiweileixing != '') searchWhere.zhiweileixing = this.formSearch.zhiweileixing;
        ...
        this.$http.get(`zhiweixinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/zhiweixinxiDetail', query: params});
      },
