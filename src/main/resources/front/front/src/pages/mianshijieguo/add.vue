...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"none"}'>
    ...
    >
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}'  label="面试结果" prop="mianshijieguo">
            <el-select v-model="ruleForm.mianshijieguo" placeholder="请选择面试结果" :disabled=" false  ||ro.mianshijieguo" >
              <el-option
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="结果详情" prop="jieguoxiangqing">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="结果详情"
              v-model="ruleForm.jieguoxiangqing">
            </el-input>
          </el-form-item>
...
<script>
  export default {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='jieguoxiangqing'){
              this.ruleForm.jieguoxiangqing = obj[o];
              this.ro.jieguoxiangqing = true;
              continue;
            }
          }
        }else if(type=='edit'){
        ...
        this.mianshijieguoOptions = "通过,未通过".split(',')
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`mianshijieguo/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('mianshijieguo/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`mianshijieguo/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
